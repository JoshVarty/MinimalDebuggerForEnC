...
namespace MinimalDebuggerForEnC
{
    class Program
    {
        ...
        private static void AttachToProcessAndApplyChanges()
        {
            ...
            List<CorAppDomain> appDomains = MakeGeneric<CorAppDomain>(corProcess.AppDomains);
            var firstDomain = appDomains.Single();
            List<CorAssembly> assemblies = MakeGeneric<CorAssembly>(firstDomain.Assemblies);

            CorAssembly sampleProcessAssembly = assemblies.Where(n => n.Name.EndsWith("SampleProcess.exe")).Single();
            CorModule sampleProcessModule = MakeGeneric<CorModule>(sampleProcessAssembly.Modules).Single();
        ...
        private static List<T> MakeGeneric<T>(IEnumerable enumerable)
        {
            var list = new List<T>();
