...
namespace MinimalDebuggerForEnC
{
    class Program
    {
        ...
        private static void AttachToProcessAndApplyChanges()
        {
            ...
            var sampleProcessAssembly = assemblies.Where(n => n.Name.EndsWith("SampleProcess.exe")).Single();
            var sampleProcessModule = MakeGeneric<CorModule>(sampleProcessAssembly.Modules).Single();
